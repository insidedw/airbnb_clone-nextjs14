...
export function ListingCard({photo, description, country: countryProp, price,userId, favoriteId, isInFavoriteList,homeId,pathName}:Props) {
    ...
    return (
        <div className={'flex flex-col'}>
            <div className={'relative h-72'}>
                <Image src={`https://firyawusvryqiujwcoma.supabase.co/storage/v1/object/public/images/${photo}`} alt={'image of house'} fill={true} className={'rounded-lg h-full object-cover'}/>

                {userId && <div className={'z-10 absolute top-2 right-2'}>
                    {isInFavoriteList ? <form action={deleteFavorite}>
                        <input type={'hidden'} name={'favoriteId'} value={favoriteId}/>
                        <input type={'hidden'} name={'userId'} value={userId}/>
                        <input type={'hidden'} name={'pathName'} value={pathName}/>
                        <DeleteToFavoriteButton/>
                    </form>: <form action={addToFavorite}>
